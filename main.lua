...
Http.post(API_URL, postData, headers, "", function(code, body)
    ...
    -- checking.setText("SERVER RAW:\n"..tostring(body)) -- debug (you can remove later)

    local clean = tostring(body):match("{.*}")
    if not clean then
      checking.setText("Server blocked request")
      return
    end

    local ok, json = pcall(cjson.decode, clean)
    if not ok then
      checking.setText("Invalid server JSON")
      return
    end
